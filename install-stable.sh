...
if ! command -v node >/dev/null 2>&1 || (( $(node -p 'Number(process.versions.node.split(".")[0])') < 22 )); then
  ...
  log_ok "Vorhandenes Node.js $(node --version) verwendet"
fi

log_step "yt-dlp ${YTDLP_VERSION} installieren und SHA-256 prüfen"
TMP_YTDLP="$(mktemp)"
...
log_ok "yt-dlp und FFmpeg bereit"

log_step "MusikBot187-Dienst vorbereiten"
...
if $SUDO systemctl is-active --quiet musikbot187; then
  log_ok "MusikBot187-Dienst läuft"
else
  log_error "MusikBot187 konnte nicht gestartet werden."
