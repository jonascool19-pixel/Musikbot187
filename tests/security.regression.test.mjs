...
async function findPort() {
  const net = await import("node:net"); const server = net.createServer(); await new Promise(resolve => server.listen(0, "127.0.0.1", resolve)); const port = server.address().port; await new Promise(resolve => server.close(resolve)); return port;
}
function request(port, path, { method = "GET", headers = {}, body = undefined } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({ hostname:"127.0.0.1", port, path, method, agent:false, headers:{ Connection:"close", ...(data ? { "content-type":"application/json", "content-length":Buffer.byteLength(data) } : {}), ...headers } }, res => {
      let text=""; res.setEncoding("utf8"); res.on("data", c => { text += c; }); res.on("end", () => { let parsed={}; try { parsed=JSON.parse(text||"{}"); } catch {} resolve({ status:res.statusCode, body:parsed }); });
...
test("first-run setup requires the installer token, preserves roles and protects admin actions", async () => {
  const dataDir = await mkdtemp(join(tmpdir(), "musikbot187-security-")); const port = await findPort(); const token="test-setup-token-1234567890";
  ...
  try {
    ...
    assert.equal((await request(port,"/api/state",{headers:auth})).status,200);
    const users=await request(port,"/api/users",{headers:auth}); assert.equal(users.status,200); assert.equal(users.body.find(x=>x.name==="normal").role,"user");
