...
test('update version comparison is numeric and bounded to the official repository',async()=>{
  ...
  let requested='';const result=await updateStatus(async url=>{requested=url;return {ok:true,status:200,json:async()=>({version:'1.8.28'})}});
  assert.equal(requested,latestPackageUrl);
  assert.deepEqual(result,{current:appVersion,latest:'1.8.28',available:false,source:'GitHub main'});
