...
test('autoplay API saves configuration, fills the queue and clears it from the dashboard switch',async t=>{
  ...
  response=await app.inject({url:'/api/autoplay',headers});assert.deepEqual(response.json().profile.blockedStyles,[]);response=await app.inject({method:'PUT',url:'/api/autoplay/profile/styles',headers,payload:{preferredStyles:['Uptempo'],blockedStyles:['Schlager']}});assert.equal(response.statusCode,200,response.body);assert.deepEqual(response.json().profile.preferredStyles,['Uptempo']);assert.deepEqual(response.json().profile.blockedStyles,['Schlager']);const persistedProfile=JSON.parse(await fs.readFile(path.join(dir,'state.json'),'utf8')).listeningProfile;assert.deepEqual(persistedProfile.preferredStyles,['Uptempo']);assert.deepEqual(persistedProfile.blockedStyles,['Schlager']);response=await app.inject({method:'PUT',url:'/api/autoplay/profile/styles',headers,payload:{preferredStyles:[],blockedStyles:Array.from({length:21},(_,index)=>`Stil ${index}`)}});assert.equal(response.statusCode,400,response.body);
