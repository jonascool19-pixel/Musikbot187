...
test('older listening profiles gain a separate empty artist preference list',async t=>{const dir=await fs.mkdtemp(path.join(os.tmpdir(),'musikbot-store-artists-')),file=path.join(dir,'state.json');t.after(()=>fs.rm(dir,{recursive:true,force:true}));await fs.writeFile(file,JSON.stringify({version:2,settings:{},listeningProfile:{version:2,tracks:[],preferredStyles:['Techno'],blockedStyles:['Schlager']}}));const store=await new Store(file).load();assert.deepEqual(store.data.listeningProfile.preferredStyles,['Techno']);assert.deepEqual(store.data.listeningProfile.preferredArtists,[]);assert.deepEqual(store.data.listeningProfile.blockedStyles,['Schlager']);});
