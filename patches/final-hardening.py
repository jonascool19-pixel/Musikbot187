...
# Shared instance adapter for the web UI. Discord keeps its existing state API;
# TS3 is controlled through a root-owned data file consumed by the TS3 process.
ts3_control = "const TS3_CONTROL_FILE = '/var/lib/radiobot/ts3-control.json';\nconst TS3_STATUS_FILE = '/var/lib/radiobot/ts3-status.json';\nconst TS3_ENV_FILE = '/etc/radiobot/ts3.env';\nfunction ts3Enabled() { return fs.existsSync(TS3_ENV_FILE); }\nfunction readTs3Status() { try { return JSON.parse(fs.readFileSync(TS3_STATUS_FILE, 'utf8')); } catch { return { enabled: ts3Enabled(), connected: false, host: '', channel: '', paused: false, volume: 80, queue: [] }; } }\nfunction writeTs3Command(command: any) { const tmp = `${TS3_CONTROL_FILE}.${process.pid}.tmp`; fs.writeFileSync(tmp, JSON.stringify({ id: crypto.randomUUID(), ...command }), { mode: 0o600 }); fs.renameSync(tmp, TS3_CONTROL_FILE); return { ok: true }; }\n"
