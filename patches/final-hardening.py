...
s=s.replace("Body: { guildId: string; mode: 'off' | 'one' | 'all'; shuffle: boolean }","Body: { playlistId?: string; mode: 'off' | 'one' | 'all'; shuffle: boolean }",1)
s=s.replace("s.activePlaylistId = req.body.guildId || s.activePlaylistId;","s.activePlaylistId = req.body.playlistId || s.activePlaylistId;",1)
s=s.replace("if (replace) { await stopGuild(guildId); state.activePlaylistId = playlist.id; state.queue = items; }","if (replace) { await stopGuild(guildId); state.manualStop = false; state.activePlaylistId = playlist.id; state.queue = items; }",1)
if "/api/state/:id/youtube" not in s:
    needle="app.post<{ Params: { id: string }; Body: { file: string; append?: boolean } }>('/api/state/:id/file'"
    route="app.post<{ Params: { id: string }; Body: { value: string; label?: string; append?: boolean } }>('/api/state/:id/youtube', async req => { const label = req.body.label?.trim() || req.body.value; const q = enqueue(req.params.id, { kind: 'youtube', value: req.body.value, label, playlistName: 'Direkt / YouTube' }, !req.body.append); if (!q.playing) await playNext(req.params.id); return q; });\n"
