...
replace_once(UI_JS,"function applyTiles(){const grid=document.querySelector('.grid');if(!grid)return;for(const t of state.layout.tiles){const el=document.querySelector(`[data-tile-id=\"${CSS.escape(t.id)}\"]`);if(!el)continue;el.style.gridColumn=`span ${t.span}`;el.style.gridRow=`span ${t.rowSpan}`;el.hidden=!t.visible;el.dataset.icon=t.icon;el.dataset.label=t.label;const title=el.querySelector('[data-tile-title]');if(title&&el.id!=='nowTitle')title.textContent=`${t.icon} ${t.label}`;el.draggable=state.editing;el.classList.toggle('builder-selected',state.editing);grid.appendChild(el);}}","function applyTiles(){const grid=document.querySelector('.grid');if(!grid)return;for(const t of state.layout.tiles){const el=document.querySelector(`[data-tile-id=\"${CSS.escape(t.id)}\"]`);if(!el)continue;el.style.gridColumn=`span ${t.span}`;el.style.gridRow=`span ${t.rowSpan}`;el.hidden=!t.visible;el.dataset.icon=t.icon;el.dataset.label=t.label;const title=el.querySelector('[data-tile-title]');if(title&&el.id!=='nowTitle')title.textContent=`${t.icon} ${t.label}`;el.draggable=false;let handle=el.querySelector(':scope > .builder-tile-handle');if(!handle){handle=document.createElement('div');handle.className='builder-tile-handle';handle.textContent='☷';el.insertBefore(handle,el.firstChild);}handle.draggable=state.editing;el.classList.toggle('builder-selected',state.editing);grid.appendChild(el);}}",'tile drag handle')
...
replace_once(UI_JS,"async function toggle(on){makeBuilderControls();state.editing=on;document.body.classList.toggle('builder-editing',on);$('#builderPanel').classList.toggle('open',on);prepareFields();applyFields();if(on){initDrag();updateControls();}else await saveLayout(true);}","async function toggle(on){makeBuilderControls();state.editing=on;document.body.classList.toggle('builder-editing',on);$('#builderPanel').classList.toggle('open',on);prepareFields();applyTiles();applyFields();if(on){initDrag();updateControls();}else await saveLayout(true);}",'enable tile drag')
