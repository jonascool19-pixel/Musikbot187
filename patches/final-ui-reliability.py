...
old = "async function load(){try{state.layout=ensureLayout(await api('/api/ui/layout'));}catch{state.layout=ensureLayout(null);}makeBuilderControls();applyTheme();prepareFields();applyTiles();applyFields();updateControls();const btn=document.createElement('button');"
new = "async function load(){if(document.querySelector('#layoutBuilderOpen'))return;try{state.layout=ensureLayout(await api('/api/ui/layout'));}catch{state.layout=ensureLayout(null);}makeBuilderControls();applyTheme();prepareFields();applyTiles();applyFields();updateControls();const btn=document.createElement('button');"
if old in s:
