...
# web-account setup, and a colored final status summary.
...
# curl | sudo bash feeds the script through stdin, so interactive prompts must
# read from /dev/tty instead of stdin. CI/non-interactive runs keep safe defaults.
WEB_USER="admin"
WEB_PASSWORD=""
if [[ -r /dev/tty && -w /dev/tty ]]; then
  printf '\n\033[1;36m=== MusikBot187 Ersteinrichtung ===\033[0m\n' > /dev/tty
  while :; do
    read -r -p 'Web-Benutzername: ' WEB_USER </dev/tty
    [[ -n "$WEB_USER" ]] && break
    echo 'Benutzername darf nicht leer sein.' > /dev/tty
  done
  while :; do
    read -r -s -p 'Web-Passwort (mindestens 12 Zeichen): ' WEB_PASSWORD </dev/tty
    echo > /dev/tty
    if [[ ${#WEB_PASSWORD} -ge 12 ]]; then
      read -r -s -p 'Web-Passwort wiederholen: ' WEB_PASSWORD_CONFIRM </dev/tty
      echo > /dev/tty
      [[ "$WEB_PASSWORD" == "$WEB_PASSWORD_CONFIRM" ]] && break
    fi
...
# Patch the pinned base installer without changing the pinned source commit:
# 1) make Deno installation non-interactive (-y),
# 2) inject the account chosen above into the initial radiobot.env.
...
if marker in text and 'INSTALL_WEB_USER=' not in text:
    injected = (
        marker
        + f'INSTALL_WEB_USER={shlex.quote(web_user)}\n'
...
# Colored final status summary. Credentials themselves are never printed.
if [[ -t 1 ]]; then
  GREEN=$'\033[1;32m'
  CYAN=$'\033[1;36m'
  YELLOW=$'\033[1;33m'
...
if [[ -n "$SETUP" ]]; then
  printf '%sErsteinrichtung:%s http://%s:3000/#setup=%s\n' "$YELLOW" "$RESET" "$IP" "$SETUP"
fi
if systemctl is-active --quiet radiobot.service; then
  printf '%sStatus:%s %släuft%s\n' "$CYAN" "$RESET" "$GREEN" "$RESET"
else
  printf '%sStatus:%s %snicht aktiv – bitte "radiobot status" prüfen%s\n' "$CYAN" "$RESET" "$YELLOW" "$RESET"
fi
printf '%s========================================%s\n\n' "$GREEN" "$RESET"
