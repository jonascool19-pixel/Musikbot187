...
import {classifyYouTubeAudioFailure,isYouTubeAccessBlocked,isYouTubeAgeRestricted,sharedYouTubeAccess} from './youtube-access.js';
...
export const youtubeRuntimeArgs=['--ignore-config','--socket-timeout','8','--extractor-retries','1','--js-runtimes','node','--remote-components','ejs:github','--sleep-requests','2'];
export const youtubeSearchStrategies=[['--extractor-args','youtubetab:skip=webpage'],[]];
export const youtubeClientStrategies=[[],['--extractor-args','youtube:player_client=mweb,default'],['--extractor-args','youtube:player_client=web_embedded']];
...
export function youtubeAuthenticationArgs(){const cookies=String(process.env.YOUTUBE_COOKIES_FILE||'').trim(),userAgent=String(process.env.YOUTUBE_USER_AGENT||'').trim(),args=[];if(cookies&&(path.isAbsolute(cookies)||path.win32.isAbsolute(cookies)))args.push('--cookies',cookies);if(userAgent.length>=20&&userAgent.length<=512)args.push('--user-agent',userAgent);return args;}
...
export function youtubeDownloadArgs(url,template,maxBytes){return [...youtubeRuntimeArgs,...youtubeAuthenticationArgs(),'--no-playlist','--force-ipv4','--restrict-filenames','--trim-filenames','140','--no-overwrites','--no-progress','--max-filesize',String(maxBytes),'--sleep-interval','5','--max-sleep-interval','8','-f',bestAudioFormat,'-o',template,url];}
...
export function isPermanentYouTubeResolutionError(error){return error?.code==='YOUTUBE_AGE_RESTRICTED'||/(?:video unavailable|not made this video available|not available in your country|private video|members[- ]only|age[- ]restricted|copyright|blocked due to|no longer available)/i.test(String(error?.message||error||''));}
...
async function resolveYoutube(q,signal,{deadline=Date.now()+youtubeResolveTimeoutMs}={}){const failures=[];let providerFailure=null;for(const strategy of youtubeClientStrategies){const remaining=deadline-Date.now();if(remaining<=0)break;try{const out=await run('yt-dlp',[...youtubeRuntimeArgs,...youtubeAuthenticationArgs(),'--no-playlist','--print',youtubePlaybackPrintTemplate,'-f',bestAudioFormat,'--force-ipv4',...strategy,q],{signal,timeout:Math.min(youtubeAttemptTimeoutMs,remaining)}),resolved=parseResolvedYouTubeOutput(out);await assertSafeExternalUrl(resolved.url);return resolved;}catch(error){if(error.name==='AbortError'||isYouTubeAccessBlocked(error))throw error;const outage=classifyYouTubeAudioFailure(error?.message);if(outage?.code==='YOUTUBE_TOKEN_PROVIDER_UNAVAILABLE')providerFailure=outage;if(outage?.code==='YOUTUBE_AGE_RESTRICTED')failures.push(outage.message);else failures.push(error.message);if(outage?.code==='YOUTUBE_AGE_RESTRICTED'||isPermanentYouTubeResolutionError(error))break;}}if(providerFailure)throw providerFailure;throw new Error(`YouTube-Audio konnte mit keiner Clientvariante aufgelöst werden. ${failures.at(-1)||''}`.trim());}
...
const spotifyMatchReasonLabels=Object.freeze({artist:'falscher Künstler',version:'falsche Version',title:'abweichender Titel',duration:'unpassende Länge',source:'nicht erreichbare Quelle',age:'altersbeschränkte Quelle',search:'fehlgeschlagene YouTube-Suche'});
...
export async function resolveSpotify(item,signal,{search=youtubeSearch,resolve=resolveYoutube}={}){
  ...
  const resolveFailureReason=error=>/unpassende Länge|verifizierte Dauer/i.test(String(error?.message||''))?'duration':isYouTubeAgeRestricted(error)?'age':'source';
