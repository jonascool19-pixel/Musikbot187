...
const blockedV4 = ip => { const p=ip.split('.').map(Number); return p.length!==4||p.some(value=>!Number.isInteger(value)||value<0||value>255)||p[0]===0||p[0]===10||p[0]===127||p[0]>=224||(p[0]===169&&p[1]===254)||(p[0]===172&&p[1]>=16&&p[1]<=31)||(p[0]===192&&p[1]===168)||(p[0]===100&&p[1]>=64&&p[1]<=127)||(p[0]===192&&p[1]===0&&(p[2]===0||p[2]===2))||(p[0]===198&&(p[1]===18||p[1]===19))||(p[0]===198&&p[1]===51&&p[2]===100)||(p[0]===203&&p[1]===0&&p[2]===113); };
const blockedV6 = ip => {const value=String(ip||'').toLowerCase();return value==='::'||value==='::1'||value.startsWith('::')||/^(?:fc|fd)/.test(value)||/^fe[89a-f]/.test(value)||value.startsWith('ff')||value.startsWith('100:')||value.startsWith('2001:2:')||value.startsWith('2001:db8:');};
export function isBlockedAddress(address){const value=String(address||'').replace(/^\[|\]$/g,''),family=net.isIP(value);return family===4?blockedV4(value):family===6?blockedV6(value):true;}
export async function assertSafeExternalUrl(raw) {
  ...
  const hostname=url.hostname.replace(/^\[|\]$/g,''),addresses=net.isIP(hostname)?[{address:hostname}]:await dns.lookup(hostname,{all:true,verbatim:true});
  if(!addresses.length||addresses.some(({address})=>isBlockedAddress(address))) throw new Error('Privates oder reserviertes Medienziel blockiert');
  return url;
}
...
export function safeMusicRelativePath(root, relative){const value=String(relative||'').replaceAll('\\','/'),parts=value.split('/');if(!parts.length||parts.length>2||parts.some(part=>!part||part==='.'||part==='..'||!/^[\p{L}\p{N} _().\-[\]]{1,180}$/u.test(part)))throw new Error('Ungültiger Musikpfad');const base=path.resolve(root),target=path.resolve(base,...parts),within=path.relative(base,target);if(!within||within.startsWith('..')||path.isAbsolute(within))throw new Error('Pfad außerhalb des Musikverzeichnisses');return target;}
