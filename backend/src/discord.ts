...
export class DiscordInstance {
  ...
  constructor(cfg: any, playlistProvider: () => any[] = () => []) {
    ...
    if (cfg.messageContentIntent === true) this.client.on('messageCreate', m => this.onMessage(m).catch(e => console.error('Discord command', e)));
    this.player.on(AudioPlayerStatus.Idle, () => { void this.next(); });
    this.player.on('error', error => { this.lastError = `Audio-Player: ${error.message}`; console.error(`Discord audio ${cfg.name}`, error); });
  }
  ...
  async next() {
    ...
    try {
      ...
      console.error(`Discord playback ${this.cfg.name}`, error);
    } finally {
