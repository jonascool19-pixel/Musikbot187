...
export async function load() {
  ...
  try {
    ...
    if (error?.code !== "ENOENT") throw new Error(`Datenbank konnte nicht gelesen werden: ${error instanceof Error ? error.message : String(error)}`);
    state = defaults();
    await save();
  }
...
export function validateCredentialInput(name, password) {
  const normalized = String(name || "").trim();
  const secret = String(password || "");
  if (!normalized || normalized.length > 64) throw new Error("Benutzername muss 1–64 Zeichen lang sein");
  if (secret.length < 5 || secret.length > 256) throw new Error("Passwort muss 5–256 Zeichen lang sein");
  return { name: normalized, password: secret };
}
...
export function createUser(name, password, role = "user") {
  const credentials = validateCredentialInput(name, password);
  if (db().users.some((u) => normalizeName(u.name) === normalizeName(credentials.name))) throw new Error("Benutzername bereits vorhanden");
  const u = { id: randomUUID(), name: credentials.name, hash: hash(credentials.password), role: role === "admin" ? "admin" : "user" };
  db().users.push(u);
  return publicUser(u);
}
...
export function login(name, password, clientKey='unknown') {
  let credentials;
  try { credentials = validateCredentialInput(name, password); } catch { return null; }
  const normalizedName = normalizeName(credentials.name);
  ...
  if (!u || !check(credentials.password, u.hash)) {
    attempt.count += 1;
    if (attempt.count >= 5) attempt.blockedUntil = now + 15 * 60 * 1000;
    loginAttempts.set(key, attempt);
    return null;
  }
  if (isLegacyHash(u.hash)) { u.hash = hash(credentials.password); void save(); }
  loginAttempts.delete(key);
  const ownSessions = [...sessions.values()].filter((session) => session.userId === u.id && session.expires > now);
  if (ownSessions.length >= SESSION_LIMIT_PER_USER) return null;
  const token = `${randomUUID()}${randomUUID()}`;
  sessions.set(token, { userId: u.id, expires: now + SESSION_MAX_AGE_MS, createdAt: now });
...
export function changePassword(userId, currentPassword, newPassword) {
  const credentials = validateCredentialInput("user", newPassword);
  if (credentials.password !== String(newPassword || "")) throw new Error("Ungültiges neues Passwort");
  const u = db().users.find((x) => x.id === userId);
  if (!u || !check(String(currentPassword || ""), u.hash)) throw new Error("Aktuelles Passwort ist falsch");
  u.hash = hash(credentials.password);
  logoutUser(userId);
  return true;
}
export function user(header) {
