...
function normalizeItem(item) {
  ...
  if (item.source === "radio" && /^https?:\/\/streams\.bigfm\.de\/bigfm-rlp-128-aac\/?$/i.test(value)) {
...
export class Player extends EventEmitter {
  ...
  async ensureEgressProxy() {
    if (this.proxyServer) return this.egressProxy;
    this.proxyServer = await new EgressProxy().start();
    this.egressProxy = this.proxyServer.url;
  ...
  pause() {
    this.paused = true;
    for (const child of [this.resolver, this.ff]) if (child) try { child.kill("SIGSTOP"); } catch {}
  }
  resume() {
    this.paused = false;
    for (const child of [this.resolver, this.ff]) if (child) try { child.kill("SIGCONT"); } catch {}
  }
  stop() {
    this.generation++;
  ...
  async shutdown() {
    this.stop();
    if (this.proxyServer) {
      await this.proxyServer.stop().catch(() => {});
  ...
  skip() {
    ...
    this.resolver = null;
    this.ff = null;
    if (this.current || this.queue.length) void this.next();
  }
  clear() { this.queue = []; this.emit("state"); }
  async remove(index) {
    if (Number.isInteger(index) && index >= 0 && index < this.queue.length) this.queue.splice(index, 1);
    this.emit("state");
  }
  ...
  async enqueue(items, { playNow = false } = {}) {
    ...
    if (playNow) {
      ...
      this.resolver = null;
      this.ff = null;
      this.queue = validated.slice(0, MAX_QUEUE_ITEMS);
      this.current = null;
      this.paused = false;
      void this.next();
      return;
  ...
  async playYtdlp(item, run) {
    ...
    this.ff = ff;
    let ytError = "";
    yt.stderr.on("data", d => { ytError += String(d); if (Buffer.byteLength(ytError, "utf8") > MAX_YTDLP_STDERR) ytError = ytError.slice(-MAX_YTDLP_STDERR); });
    yt.stdout.pipe(ff.stdin);
    yt.on("error", () => { try { ff.kill("SIGTERM"); } catch {} });
    yt.on("close", code => {
      if (code !== 0 && run === this.generation) {
        this.emit("diagnostic", ytError.trim().slice(0, 1000) || "yt-dlp konnte den YouTube-Stream nicht liefern");
    ...
    if (this.resolver === yt) this.resolver = null;
    if (run !== this.generation) return "cancelled";
    if (yt.exitCode !== null && yt.exitCode !== 0) return "failed";
  ...
  async playSource(rawItem, run) {
    ...
    const ff = this.spawnFn(FFMPEG, ffArgs, { stdio: ["ignore", "pipe", "pipe"] });
    this.ff = ff;
  ...
  async next() {
    ...
    try {
      ...
      if (run !== this.generation) return;
      if (result === "failed") {
        const recovered = await this.recover(item, run, "FFmpeg/yt-dlp wurde unerwartet beendet");
