...
app.get("/api/setup-link", async (request) => {
  const envUrl = process.env.MUSIKBOT187_PUBLIC_URL?.trim();
  if (envUrl) return { url: envUrl.replace(/\/$/, "") + "/" };
  const forwardedHost = String(request.headers["x-forwarded-host"] || request.headers.host || `localhost:${PORT}`);
  const forwardedProto = String(request.headers["x-forwarded-proto"] || "http").split(",")[0];
  return { url: `${forwardedProto}://${forwardedHost}/` };
});
...
app.post("/api/discord",async(request,reply)=>{if(!requireAdmin(request,reply))return;const body=request.body as Partial<DiscordInstance>;const existing=db().discord.find(entry=>entry.id===body.id);const instance:DiscordInstance={id:body.id||randomUUID(),name:String(body.name||existing?.name||"Discord"),token:body.token?String(body.token):String(existing?.token||""),clientId:String(body.clientId||existing?.clientId||""),guildId:String(body.guildId||existing?.guildId||""),channelId:String(body.channelId||existing?.channelId||""),prefix:String(body.prefix||existing?.prefix||"!")};const index=db().discord.findIndex(entry=>entry.id===instance.id);if(index>=0)db().discord[index]={...db().discord[index],...instance};else db().discord.push(instance);await saveState();return publicDiscord();});
app.delete("/api/discord/:id",async(request,reply)=>{if(!requireAdmin(request,reply))return;const id=(request.params as {id:string}).id;await discord.disconnect(id);db().discord=db().discord.filter(entry=>entry.id!==id);await saveState();return publicDiscord();});
