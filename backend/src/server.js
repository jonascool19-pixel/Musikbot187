...
function consumeLimit(key, max, windowMs) {
  const now = Date.now(); const current = limiter.get(key);
  if (!current || current.expires <= now) { limiter.set(key, { count: 1, expires: now + windowMs }); return true; }
  if (current.count >= max) return false;
  current.count += 1; return true;
}
const limiterSweep = setInterval(() => { const now = Date.now(); for (const [key, entry] of limiter) if (entry.expires <= now) limiter.delete(key); }, 5 * 60 * 1000);
limiterSweep.unref?.();
...
function normalizePlaylistItem(item) {
  ...
  const source = String(item.source || "youtube").toLowerCase(); const rawUrl = item.url.trim().slice(0, 4096); if (!rawUrl) return null;
  ...
  if (source === "file") { const dir = safeMusicDir(); const candidate = path.isAbsolute(rawUrl) ? path.relative(dir, path.resolve(rawUrl)) : rawUrl; if (!candidate || candidate.startsWith("..") || path.isAbsolute(candidate)) return null; normalized.url = path.basename(candidate); }
  return normalized;
}
async function directorySize(dir) { let total = 0; for (const item of await readdir(dir, { withFileTypes: true })) if (item.isFile()) { try { total += (await stat(path.join(dir, item.name))).size; } catch {} if (total >= MAX_MUSIC_QUOTA) break; } return total; }
function validAudioHeader(buffer, ext) { if (ext === ".wav") return buffer.length >= 12 && buffer.toString("ascii", 0, 4) === "RIFF" && buffer.toString("ascii", 8, 12) === "WAVE"; if (ext === ".flac") return buffer.toString("ascii", 0, 4) === "fLaC"; if (ext === ".ogg" || ext === ".opus") return buffer.toString("ascii", 0, 4) === "OggS"; if (ext === ".webm") return buffer.length >= 4 && buffer.readUInt32BE(0) === 0x1a45dfa3; if (ext === ".m4a") return buffer.length >= 12 && buffer.toString("ascii", 4, 8) === "ftyp"; if (ext === ".aac") return buffer.length >= 2 && buffer[0] === 0xff && (buffer[1] & 0xf6) === 0xf0; if (ext === ".mp3") return buffer.toString("ascii", 0, 3) === "ID3" || (buffer.length >= 2 && buffer[0] === 0xff && (buffer[1] & 0xe0) === 0xe0); return false; }
async function controlAction(action) { return await new Promise((resolve, reject) => { const socket = net.createConnection(CONTROL_SOCKET); let data = ""; const timer = setTimeout(() => { socket.destroy(); reject(new Error("Control-Dienst antwortet nicht")); }, 8000); socket.on("connect", () => socket.end(JSON.stringify({ action }) + "\n")); socket.on("data", chunk => { data += String(chunk); if (data.length > 8192) { socket.destroy(); reject(new Error("Ungültige Control-Antwort")); } }); socket.on("error", error => { clearTimeout(timer); reject(error); }); socket.on("close", () => { clearTimeout(timer); try { const result = JSON.parse(data.trim()); result.ok ? resolve(result) : reject(new Error(result.error || "Control-Aktion fehlgeschlagen")); } catch (error) { reject(error); } }); }); }
...
app.addHook("onRequest", async (request, reply) => { const length = Number(request.headers["content-length"] || 0); const isMultipart = String(request.headers["content-type"] || "").startsWith("multipart/form-data"); if (length > MAX_JSON_BODY && !isMultipart) return reply.code(413).send({ error: "Anfrage ist zu groß" }); const pathName = request.url.split("?")[0]; if (pathName === "/api/health") reply.header("Cache-Control", "no-store"); });
app.addHook("onSend", async (request, reply, payload) => { reply.header("X-Content-Type-Options", "nosniff"); reply.header("X-Frame-Options", "DENY"); reply.header("Referrer-Policy", "no-referrer"); reply.header("Permissions-Policy", "geolocation=(), microphone=(), camera=()"); reply.header("Cache-Control", "no-store"); if (request.url.startsWith("/api/health")) { try { return JSON.stringify({ ok: true, name: "MusikBot187" }); } catch {} } return payload; });
...
app.get("/api/setup-link", async request => { const env = process.env.MUSIKBOT187_PUBLIC_URL?.trim(); if (env) return { url: env.replace(/\/$/, "") + "/" }; return { url: `${request.protocol || "http"}://${request.hostname || "localhost"}:${PORT}/` }; });
...
app.get("/api/search", async (request, reply) => { const u = auth(request, reply); if (!u) return; const q = String(request.query?.q || "").trim().slice(0, 200); if (!consumeLimit(`search:${request.ip}:${u.id}`, 30, 60_000)) return reply.code(429).send({ error: "Zu viele Suchanfragen; bitte kurz warten" }); const source = searchSource(String(request.query?.source || "all")); if (!q) return { youtube: [], radio: [], spotify: [] }; const out = { youtube: [], radio: [], spotify: [] }; await Promise.allSettled([source === "all" || source === "youtube" ? youtubeSearch(q).then(v => { out.youtube = v; }) : Promise.resolve(), source === "all" || source === "radio" ? radioSearch(q).then(v => { out.radio = v; }) : Promise.resolve(), source === "all" || source === "spotify" ? spotifySearch(q, db().integration).then(v => { out.spotify = v; }) : Promise.resolve()]); return out; });
app.post("/api/play", async (request, reply) => { const u = auth(request, reply); if (!u) return; if (!consumeLimit(`play:${request.ip}:${u.id}`, 60, 60_000)) return reply.code(429).send({ error: "Zu viele Wiedergabeanfragen; bitte kurz warten" }); if (!Array.isArray(request.body?.items) || request.body.items.length > 100) return reply.code(413).send({ error: "Zu viele Queue-Einträge" }); await player.enqueue(request.body.items); return player.snapshot(); });
...
app.get("/api/files", async (request, reply) => { if (!auth(request, reply)) return; const dir = safeMusicDir(); await mkdir(dir, { recursive: true }); return readdir(dir, { withFileTypes: true }).then(items => items.filter(x => x.isFile()).map(x => ({ name: x.name, directory: false, path: x.name }))); });
app.post("/api/music/upload", async (request, reply) => {
  if (!admin(request, reply)) return;
  let target = null;
  try {
    const dir = safeMusicDir(); await mkdir(dir, { recursive: true }); if ((await directorySize(dir)) >= MAX_MUSIC_QUOTA) return reply.code(413).send({ error: "Musikverzeichnis ist voll" });
    const data = await request.file(); if (!data) return reply.code(400).send({ error: "Keine Datei hochgeladen" }); target = safeMusicPath(data.filename);
    ...
    const info = await stat(target); if (!info.size || info.size > MAX_UPLOAD || (await directorySize(dir)) > MAX_MUSIC_QUOTA) { await unlink(target); return reply.code(413).send({ error: "Datei überschreitet die Speichergrenze" }); }
    const signature = await readFile(target, { encoding: null }).then(buffer => buffer.subarray(0, 16)); if (!validAudioHeader(signature, path.extname(target).toLowerCase())) { await unlink(target); return reply.code(415).send({ error: "Dateiinhalt passt nicht zur Audio-Endung" }); }
    return { ok: true, file: { name: path.basename(target), size: info.size } };
  } catch (error) { if (target) { try { await unlink(target); } catch {} } return reply.code(error?.code === "EEXIST" ? 409 : error?.code === "FST_REQ_FILE_TOO_LARGE" ? 413 : 400).send({ error: error instanceof Error ? error.message : String(error) }); }
});
...
app.put("/api/integration/spotify", async (request, reply) => { if (!admin(request, reply)) return; db().integration.spotifyClientId = String(request.body?.clientId || "").slice(0, 200); db().integration.spotifyClientSecret = await encryptSecret(String(request.body?.clientSecret || "").slice(0, 256)); await save(); return { ok: true }; });
...
app.put("/api/ts3", async (request, reply) => { if (!admin(request, reply)) return; const incoming = Array.isArray(request.body?.instances) ? request.body.instances.slice(0, 16) : []; db().ts3 = await Promise.all(incoming.map(async x => { const old = db().ts3.find(y => y.id === x.id); return { id: String(x.id || randomUUID()).slice(0, 80), name: String(x.name || "TS3").slice(0, 128), enabled: x.enabled !== false, host: String(x.host || "").trim().slice(0, 255), port: Number(x.port || 9987), nickname: String(x.nickname || "MusikBot187").trim().slice(0, 64), channel: String(x.channel || "").trim().slice(0, 255), password: x.password ? await encryptSecret(String(x.password).slice(0, 256)) : String(old?.password || "") }; })); await save(); return publicTS3(); });
