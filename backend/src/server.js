...
function requireAdmin(request, reply) { const u = admin(request, reply); return !!u; }
...
app.post("/api/play/:action", async (request, reply) => { if (!auth(request, reply)) return; const action = String(request.params.action || ""); const body = request.body || {}; if (action === "pause") player.pause(); else if (action === "resume") player.resume(); else if (action === "stop") player.stop(); else if (action === "skip") player.skip(); else if (action === "clear") player.clear(); else if (action === "volume") { const value = Number(body.value); if (!Number.isFinite(value)) return reply.code(400).send({ error: "Ungültige Lautstärke" }); player.setVolume(value); } else if (action === "mode" && ["queue", "repeat", "shuffle"].includes(body.mode)) player.setMode(body.mode); else return reply.code(400).send({ error: "Ungültige Player-Aktion" }); db().settings.volume = player.volume; db().settings.mode = player.mode; await save(); return player.snapshot(); });
...
app.put("/api/settings", async (request, reply) => { if (!requireAdmin(request, reply)) return; const b = request.body || {}; if (typeof b.volume === "number") player.setVolume(b.volume); if (typeof b.mode === "string") { if (!["queue", "repeat", "shuffle"].includes(b.mode)) return reply.code(400).send({ error: "Ungültiger Modus" }); player.setMode(b.mode); } if (typeof b.outputType === "string") { if (!["discord", "ts3", "none"].includes(b.outputType)) return reply.code(400).send({ error: "Ungültiges Ausgabeziel" }); db().settings.outputType = b.outputType; } if (typeof b.outputId === "string") db().settings.outputId = b.outputId; if (typeof b.networkInterface === "string") db().settings.networkInterface = b.networkInterface; if (typeof b.filesDirectory === "string" && b.filesDirectory.trim()) { db().settings.filesDirectory = path.resolve(b.filesDirectory.trim()); await mkdir(db().settings.filesDirectory, { recursive: true }); } if (typeof b.theme === "string") db().settings.theme = b.theme; db().settings.volume = player.volume; db().settings.mode = player.mode; await save(); return db().settings; });
app.put("/api/integration/spotify", async (request, reply) => { if (!requireAdmin(request, reply)) return; db().integration.spotifyClientId = String(request.body?.clientId || ""); db().integration.spotifyClientSecret = String(request.body?.clientSecret || ""); await save(); return { ok: true }; });
app.get("/api/users", async (request, reply) => { if (!requireAdmin(request, reply)) return; return db().users.map(({ id, name, role }) => ({ id, name, role })); });
app.post("/api/users", async (request, reply) => { if (!requireAdmin(request, reply)) return; const name = String(request.body?.name || "").trim(); const password = String(request.body?.password || ""); if (!name || password.length < 5) return reply.code(400).send({ error: "Name und Passwort erforderlich; Passwort mindestens 5 Zeichen" }); const role = request.body?.role === "user" ? "user" : "admin"; if (db().users.some(x => x.name === name)) return reply.code(409).send({ error: "Benutzername bereits vorhanden" }); db().users.push({ id: randomUUID(), name, hash: scryptSync(password, "musikbot187", 32).toString("hex"), role }); await save(); return { ok: true }; });
...
app.post("/api/discord", async (request, reply) => { if (!requireAdmin(request, reply)) return; const b = request.body || {}; const old = db().discord.find(x => x.id === b.id); const x = { id: String(b.id || randomUUID()), name: String(b.name || old?.name || "Discord"), enabled: b.enabled !== false, token: b.token ? String(b.token) : String(old?.token || ""), clientId: String(b.clientId || old?.clientId || ""), guildId: String(b.guildId || old?.guildId || ""), channelId: String(b.channelId || old?.channelId || ""), prefix: String(b.prefix || old?.prefix || "!") }; if (x.clientId && !/^\d{17,20}$/.test(x.clientId)) return reply.code(400).send({ error: "Discord Client-ID muss aus 17–20 Ziffern bestehen" }); setDiscord(x); await save(); return publicDiscord(); });
app.delete("/api/discord/:id", async (request, reply) => { if (!requireAdmin(request, reply)) return; await discord.disconnect(request.params.id); db().discord = db().discord.filter(x => x.id !== request.params.id); await save(); return publicDiscord(); });
app.post("/api/discord/:id/connect", async (request, reply) => { if (!requireAdmin(request, reply)) return; const x = db().discord.find(y => y.id === request.params.id); if (!x) return reply.code(404).send({ error: "Instanz nicht gefunden" }); try { await discord.connect(x); return discord.status(); } catch (e) { const m = e instanceof Error ? e.message : String(e); recordDiagnostic(`Discord ${x.name}: ${m}`); return reply.code(400).send({ error: m }); } });
app.post("/api/discord/:id/disconnect", async (request, reply) => { if (!requireAdmin(request, reply)) return; await discord.disconnect(request.params.id); return discord.status(); });
app.post("/api/discord/:id/join", async (request, reply) => { if (!requireAdmin(request, reply)) return; try { await discord.join(request.params.id); db().settings.outputType = "discord"; db().settings.outputId = request.params.id; await save(); return discord.status(); } catch (e) { const m = e instanceof Error ? e.message : String(e); recordDiagnostic(`Discord Voice ${request.params.id}: ${m}`); return reply.code(400).send({ error: m }); } });
app.get("/api/discord/:id/guilds", async (request, reply) => { if (!requireAdmin(request, reply)) return; return discord.guilds(request.params.id); });
app.get("/api/discord/:id/guilds/:guildId/channels", async (request, reply) => { if (!requireAdmin(request, reply)) return; return discord.channels(request.params.id, request.params.guildId); });
app.get("/api/ts3", async (request, reply) => { if (!auth(request, reply)) return; return publicTS3(); });
app.put("/api/ts3", async (request, reply) => { if (!requireAdmin(request, reply)) return; db().ts3 = Array.isArray(request.body?.instances) ? request.body.instances : []; await save(); return publicTS3(); });
app.post("/api/ts3/:id/connect", async (request, reply) => { if (!requireAdmin(request, reply)) return; const x = db().ts3.find(y => y.id === request.params.id); if (!x) return reply.code(404).send({ error: "Instanz nicht gefunden" }); try { await ts3.connect(x); db().settings.outputType = "ts3"; db().settings.outputId = x.id; await save(); return publicTS3(); } catch (e) { const m = e instanceof Error ? e.message : String(e); recordDiagnostic(`TS3 ${x.name}: ${m}`); return reply.code(400).send({ error: m }); } });
app.post("/api/ts3/:id/disconnect", async (request, reply) => { if (!requireAdmin(request, reply)) return; await ts3.disconnect(request.params.id); return publicTS3(); });
app.post("/api/control", async (request, reply) => { if (!requireAdmin(request, reply)) return; const action = String(request.body?.action || ""); if (!["restart-bot", "stop-bot", "restart-system", "shutdown-system"].includes(action)) return reply.code(400).send({ error: "Ungültige Aktion" }); await exec("sudo", ["/usr/local/sbin/musikbot187-control", action]); return { ok: true }; });
