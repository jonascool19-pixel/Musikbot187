...
function validSetupToken(provided) {
  if (!SETUP_TOKEN || !provided) return false;
  const a = Buffer.from(SETUP_TOKEN, "utf8");
...
app.post("/api/setup", async (request, reply) => {
  ...
  if (!setupAuthorized(request)) return reply.code(403).send({ error: "Ungültiger Einrichtungslink" });
...
app.get("/api/search", async (request, reply) => {
  ...
  const source = searchSource(String(request.query?.source || "all"));
...
app.post("/api/play/:action", async (request, reply) => {
  ...
  else if (action === "volume") { const value = Number(body.value); if (!Number.isFinite(value)) return reply.code(400).send({ error: "Ungültige Lautstärke" }); player.setVolume(value); }
  else if (action === "mode") { if (!["queue", "repeat", "shuffle"].includes(body.mode)) return reply.code(400).send({ error: "Ungültiger Modus" }); player.setMode(body.mode); }
  else return reply.code(400).send({ error: "Ungültige Player-Aktion" });
  db().settings.volume = player.volume; db().settings.mode = player.mode; await save(); return player.snapshot();
});
...
app.get("/api/files", async (request, reply) => {
  ...
  const dir = path.resolve(db().settings.filesDirectory);
  await mkdir(dir, { recursive: true });
  return readdir(dir, { withFileTypes: true }).then(items => items.map(x => ({ name: x.name, directory: x.isDirectory(), path: path.join(dir, x.name) })));
});
...
app.put("/api/settings", async (request, reply) => {
  ...
  if (typeof b.mode === "string") { if (!["queue", "repeat", "shuffle"].includes(b.mode)) return reply.code(400).send({ error: "Ungültiger Modus" }); player.setMode(b.mode); }
  if (typeof b.outputType === "string") { if (!["discord", "ts3", "none"].includes(b.outputType)) return reply.code(400).send({ error: "Ungültiges Ausgabeziel" }); db().settings.outputType = b.outputType; }
  ...
  if (typeof b.filesDirectory === "string" && b.filesDirectory.trim()) { db().settings.filesDirectory = path.resolve(b.filesDirectory.trim()); await mkdir(db().settings.filesDirectory, { recursive: true }); }
  if (typeof b.theme === "string") db().settings.theme = b.theme;
  db().settings.volume = player.volume; db().settings.mode = player.mode; await save(); return db().settings;
});
