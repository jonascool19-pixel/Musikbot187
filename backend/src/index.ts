...
function requireAdmin(request: any, reply: any) {
  const user = auth(request, reply);
  if (!user) return null;
  if (user.role !== 'admin') { reply.code(403).send({ error: 'Administratorrechte erforderlich.' }); return null; }
  return user;
}
function save() { writeConfig(config); }
function bot(id = config.activeInstance): any { return discord.get(id) ?? ts3.get(id); }
function publicInstance(x: any, live: any) {
  const safe = { ...x };
  delete safe.token;
  delete safe.clientSecret;
  delete safe.serverPassword;
  delete safe.channelPassword;
  return { ...safe, ...(live?.state?.() ?? {}) };
}
function systemStatus() {
  const totalMem = os.totalmem();
  const freeMem = os.freemem();
  const load = os.loadavg()[0];
  const cpuPercent = Math.max(0, Math.min(100, Math.round((load / Math.max(os.cpus().length, 1)) * 100)));
  return {
...
app.post('/api/setup/user', async (request: any, reply: any) => {
  if (config.users.length) return reply.code(409).send({ error: 'Benutzer existiert bereits.' });
...
app.post('/api/auth/login', async (request: any, reply: any) => {
  if (!config.users.length) return reply.code(403).send({ error: 'Zuerst den ersten Benutzer anlegen.' });
  const body = request.body ?? {};
  const username = String(body.username ?? '');
  const password = String(body.password ?? '');
  const user = config.users.find((u: any) => u.username === username);
  if (!user || !verifyPassword(password, user.salt, user.hash)) return reply.code(401).send({ error: 'Anmeldung fehlgeschlagen.' });
...
app.post('/api/auth/logout', async (request: any, reply: any) => {
  const token = cookieToken(request); if (token) sessions.delete(token);
...
app.get('/api/me', async (request: any, reply: any) => {
  ...
  return { user: { id: user.id, username: user.username, role: user.role }, setupComplete: config.setupComplete, activeInstance: config.activeInstance };
});
...
app.get('/api/settings', async (request: any, reply: any) => {
  ...
  return {
    ...
    instances: {
      discord: (config.instances.discord ?? []).map((x: any) => { const v = { ...x }; delete v.token; return v; }),
      ts3: config.instances.ts3 ?? [],
      spotify: (config.instances.spotify ?? []).map((x: any) => { const v = { ...x }; delete v.clientSecret; return v; })
    }
  };
});
...
app.put('/api/settings', async (request: any, reply: any) => {
  ...
  if (Array.isArray(body.discord) && user.role !== 'viewer') {
    const existing = new Map((config.instances.discord ?? []).map((x: any) => [x.id, x]));
    config.instances.discord = body.discord.map((x: any, i: number) => {
      const prev: any = existing.get(x.id);
      return { id: x.id || `discord-${i + 1}`, name: x.name || `Discord ${i + 1}`, token: x.token || prev?.token || '', guildId: x.guildId || prev?.guildId || '', voiceChannelId: x.voiceChannelId || prev?.voiceChannelId || '', prefix: x.prefix || '!' };
    });
  }
  if (Array.isArray(body.ts3) && user.role !== 'viewer') {
    const existing = new Map((config.instances.ts3 ?? []).map((x: any) => [x.id, x]));
    config.instances.ts3 = body.ts3.map((x: any, i: number) => {
      const prev: any = existing.get(x.id);
      return { id: x.id || `ts3-${i + 1}`, name: x.name || `TS3 ${i + 1}`, host: x.host || '', nickname: x.nickname || 'RadioBot TS3', channel: x.channel || '', channelPassword: x.channelPassword || '', serverPassword: x.serverPassword || '', identity: x.identity || prev?.identity || '' };
    });
  }
