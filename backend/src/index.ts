...
client.on('interactionCreate', async interaction => {
  try {
    if (interaction.isButton() && interaction.customId.startsWith('rb:')) {
      ...
      const state = enqueue(interaction.guildId, { kind: item.kind as SourceKind, value: item.value, label: item.label, playlistName: 'Direkt' }); if (!state.playing) await playNext(interaction.guildId); return void interaction.reply(`▶️ **${item.label}**`);
    }
    ...
    switch (interaction.commandName) {
      ...
      case 'play': {
        ...
        if (isYoutubeUrl(query)) item = { kind: 'youtube', value: query, label: query, meta: 'YouTube' }; else { const found = await unifiedSearch(query); item = [...found.local, ...found.radios, ...found.youtube].find((x): x is SearchItem & { kind: SourceKind } => x.kind !== 'spotify'); }
        if (!item) return void interaction.reply('Nichts direkt abspielbares gefunden.'); await stopGuild(interaction.guildId); enqueue(interaction.guildId, { kind: item.kind as SourceKind, value: item.value, label: item.label, playlistName: 'Direkt' }, true); await playNext(interaction.guildId); return void interaction.reply(`▶️ **${item.label}**`);
      }
      case 'playlist': {
