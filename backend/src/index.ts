...
let spotifyOAuthState: string | undefined;
...
function checkAuth(req: any, reply: any): boolean {
  ...
  const user = separator >= 0 ? decoded.slice(0, separator) : ''; const password = separator >= 0 ? decoded.slice(separator + 1) : '';
...
function makePlayer(guildId: string) {
  ...
  if (!player) {
    ...
    player.on(AudioPlayerStatus.Idle, () => { currentProcesses.get(guildId)?.kill('SIGTERM'); currentProcesses.delete(guildId); playNext(guildId).catch(console.error); });
...
function sourceToPath(source: string): string {
  const name = source.slice(5); const resolved = path.resolve(MUSIC_DIR, name);
...
async function ensureConnection(guildId: string) {
  ...
  conn.on('error', error => console.error('voice error', guildId, error)); connections.set(guildId, conn); conn.subscribe(makePlayer(guildId)); return conn;
}
async function playNext(guildId: string) {
  const state = getState(guildId); const source = state.queue.shift();
  if (!source) { state.playing = undefined; state.playingType = undefined; saveJson(DB_FILE, db); return; }
  state.playing = source.startsWith('file:') ? source.slice(5) : source; state.playingType = source.startsWith('file:') ? 'file' : 'radio'; state.paused = false; saveJson(DB_FILE, db);
  const conn = await ensureConnection(guildId); const player = makePlayer(guildId); const input = source.startsWith('file:') ? sourceToPath(source) : source;
  const args = source.startsWith('file:') ? ['-hide_banner','-loglevel','error','-i',input,'-vn','-f','s16le','-ar','48000','-ac','2','pipe:1'] : ['-hide_banner','-loglevel','error','-reconnect','1','-reconnect_streamed','1','-reconnect_delay_max','5','-i',input,'-vn','-f','s16le','-ar','48000','-ac','2','pipe:1'];
  const ff = spawn('ffmpeg', args, { stdio: ['ignore','pipe','inherit'] }); currentProcesses.set(guildId, ff); ff.on('exit', () => { if (currentProcesses.get(guildId) === ff) currentProcesses.delete(guildId); });
  const resource = createAudioResource(ff.stdout, { inputType: StreamType.Raw, inlineVolume: true }); resource.volume?.setVolume(Math.max(0, Math.min(2, state.volume / 100))); conn.subscribe(player); player.play(resource);
}
async function stopGuild(guildId: string) {
  const state = getState(guildId); state.queue=[]; state.playing=undefined; state.playingType=undefined; players.get(guildId)?.stop(true); currentProcesses.get(guildId)?.kill('SIGTERM'); currentProcesses.delete(guildId); connections.get(guildId)?.destroy(); connections.delete(guildId); saveJson(DB_FILE, db);
}
async function spotifyToken(): Promise<string> {
  ...
  if (!response.ok) throw new Error(`Spotify token refresh fehlgeschlagen (${response.status})`); const data = await response.json() as any; spotify.accessToken=data.access_token; spotify.expiresAt=Date.now()+Number(data.expires_in??3600)*1000; if(data.refresh_token)spotify.refreshToken=data.refresh_token; saveJson(SPOTIFY_FILE,spotify); return spotify.accessToken;
}
async function spotifyApi(endpoint: string, init: RequestInit = {}, retry = true): Promise<any> {
  ...
  const response=await fetch(`https://api.spotify.com/v1${endpoint}`,{...init,headers}); if(response.status===401&&retry&&spotify.refreshToken){spotify.expiresAt=0;await spotifyToken();return spotifyApi(endpoint,init,false);} if(!response.ok){const text=await response.text();throw new Error(`Spotify API ${response.status}: ${text}`);} if(response.status===204)return null; return response.json();
}
function spotifyPlaylistId(input: string): string | null { const match=input.match(/(?:open\.spotify\.com\/playlist\/|spotify:playlist:)([A-Za-z0-9]+)/); return match?.[1]??null; }

const client = new Client({ intents:[GatewayIntentBits.Guilds,GatewayIntentBits.GuildVoiceStates] });
const commands=[new SlashCommandBuilder().setName('radio').setDescription('Radio starten').addStringOption(o=>o.setName('name').setDescription('Sendername').setRequired(true)),new SlashCommandBuilder().setName('stop').setDescription('Wiedergabe stoppen'),new SlashCommandBuilder().setName('skip').setDescription('Nächste Quelle abspielen'),new SlashCommandBuilder().setName('volume').setDescription('Lautstärke setzen').addIntegerOption(o=>o.setName('percent').setDescription('0-100').setRequired(true).setMinValue(0).setMaxValue(100)),new SlashCommandBuilder().setName('join').setDescription('Bot in deinen Voice-Channel holen')].map(c=>c.toJSON());
...
client.on('interactionCreate',async interaction=>{if(!interaction.isChatInputCommand()||!interaction.guildId)return;const state=getState(interaction.guildId);try{if(interaction.commandName==='join'){const member=await interaction.guild!.members.fetch(interaction.user.id);if(!member.voice.channel||member.voice.channel.type!==ChannelType.GuildVoice)return void interaction.reply('Du musst selbst in einem Voice-Channel sein.');state.voiceChannelId=member.voice.channel.id;saveJson(DB_FILE,db);await ensureConnection(interaction.guildId);return void interaction.reply(`🔊 Verbunden mit **${member.voice.channel.name}**`);}if(interaction.commandName==='stop'){await stopGuild(interaction.guildId);return void interaction.reply('⏹️ Gestoppt.');}if(interaction.commandName==='skip'){players.get(interaction.guildId)?.stop(true);return void interaction.reply('⏭️ Übersprungen.');}if(interaction.commandName==='volume'){const value=interaction.options.getInteger('percent',true);state.volume=value;saveJson(DB_FILE,db);return void interaction.reply(`🔊 Lautstärke: ${value}%`);}if(interaction.commandName==='radio'){const name=interaction.options.getString('name',true);const radio=db.radios.find(r=>r.name.toLowerCase()===name.toLowerCase());if(!radio)return void interaction.reply('Sender nicht gefunden.');state.queue=[radio.url];state.radioId=radio.id;saveJson(DB_FILE,db);await playNext(interaction.guildId);return void interaction.reply(`📻 ${radio.name}`);}}catch(error){console.error(error);if(!interaction.replied)await interaction.reply(`Fehler: ${error instanceof Error?error.message:'Unbekannt'}`);}});

const app=Fastify({logger:true});
await app.register(cors,{origin:true});
await app.register(fastifyStatic,{root:path.join(APP_ROOT,'frontend'),prefix:'/',index:['index.html']});
app.addHook('preHandler',async(req,reply)=>{if(req.url.startsWith('/api/')&&!checkAuth(req,reply))return reply;});
...
app.get('/api/radios',async()=>db.radios); app.post<{Body:{name:string;url:string}}>('/api/radios',async req=>{const radio:Radio={id:id(),name:req.body.name.trim(),url:req.body.url.trim(),enabled:true};db.radios.push(radio);saveJson(DB_FILE,db);return radio;}); app.delete<{Params:{id:string}}>('/api/radios/:id',async req=>{db.radios=db.radios.filter(r=>r.id!==req.params.id);saveJson(DB_FILE,db);return{ok:true};});
app.get('/api/media',async()=>fs.readdirSync(MUSIC_DIR).filter(f=>/\.(mp3|wav|ogg|flac|m4a)$/i.test(f)).sort());
app.get<{Params:{id:string}}>('/api/state/:id',async req=>getState(req.params.id)); app.post<{Params:{id:string};Body:{voiceChannelId:string}}>('/api/state/:id/voice',async req=>{const s=getState(req.params.id);s.voiceChannelId=req.body.voiceChannelId;saveJson(DB_FILE,db);return s;}); app.post<{Params:{id:string};Body:{radioId:string}}>('/api/state/:id/radio',async req=>{const s=getState(req.params.id);const r=db.radios.find(x=>x.id===req.body.radioId);if(!r)throw new Error('Radio nicht gefunden');s.queue=[r.url];s.radioId=r.id;saveJson(DB_FILE,db);await playNext(req.params.id);return s;}); app.post<{Params:{id:string};Body:{file:string}}>('/api/state/:id/file',async req=>{const s=getState(req.params.id);const file=sourceToPath(`file:${req.body.file}`);if(!fs.existsSync(file))throw new Error('Datei nicht gefunden');s.queue=[`file:${req.body.file}`];saveJson(DB_FILE,db);await playNext(req.params.id);return s;}); app.post<{Params:{id:string}}>('/api/state/:id/stop',async req=>{await stopGuild(req.params.id);return getState(req.params.id);}); app.post<{Params:{id:string}}>('/api/state/:id/skip',async req=>{players.get(req.params.id)?.stop(true);return getState(req.params.id);}); app.post<{Params:{id:string};Body:{volume:number}}>('/api/state/:id/volume',async req=>{const s=getState(req.params.id);s.volume=Math.max(0,Math.min(100,Number(req.body.volume)));saveJson(DB_FILE,db);return s;});
...
app.post<{Body:{uri:string;deviceId?:string}}>('/api/spotify/play',async req=>{await spotifyApi('/me/player/play'+(req.body.deviceId?`?device_id=${encodeURIComponent(req.body.deviceId)}`:''),{method:'PUT',body:JSON.stringify({uris:[req.body.uri]})});return{ok:true};}); app.post('/api/spotify/pause',async()=>{await spotifyApi('/me/player/pause',{method:'PUT'});return{ok:true};}); app.post('/api/spotify/next',async()=>{await spotifyApi('/me/player/next',{method:'POST'});return{ok:true};});
