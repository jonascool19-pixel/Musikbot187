...
async function startInstances() {
  for (const cfg of config.instances.discord ?? []) { if (discord.has(cfg.id)) continue; const instance = applyDiscordStability(new DiscordInstance(cfg)); discord.set(cfg.id, instance); instance.start().catch(e => console.error(`Discord ${cfg.name}:`, e)); }
  for (const cfg of config.instances.ts3 ?? []) { if (ts3.has(cfg.id)) continue; const instance = new Ts3Instance(cfg); ts3.set(cfg.id, instance); instance.start().catch(e => console.error(`TS3 ${cfg.name}:`, e)); }
}
...
app.put('/api/settings', async (request:any, reply:any)=>{
  const user=auth(request,reply);if(!user)return;const body=request.body??{};
  if(typeof body.activeInstance==='string') config.activeInstance=body.activeInstance;
  if(body.settings&&(user.role==='admin'||user.role==='operator')) config.settings={...config.settings,...body.settings};
  if(Array.isArray(body.discord)&&user.role!=='viewer'){const existing=new Map((config.instances.discord??[]).map((x:any)=>[x.id,x]));config.instances.discord=body.discord.map((x:any,i:number)=>{const prev:any=existing.get(x.id);return{id:x.id||`discord-${i+1}`,name:x.name||`Discord ${i+1}`,token:x.token||prev?.token||'',guildId:x.guildId||prev?.guildId||'',voiceChannelId:x.voiceChannelId||prev?.voiceChannelId||'',prefix:x.prefix||'!'};});}
  if(Array.isArray(body.ts3)&&user.role!=='viewer'){const existing=new Map((config.instances.ts3??[]).map((x:any)=>[x.id,x]));config.instances.ts3=body.ts3.map((x:any,i:number)=>{const prev:any=existing.get(x.id);return{id:x.id||`ts3-${i+1}`,name:x.name||`TS3 ${i+1}`,host:x.host||'',nickname:x.nickname||'RadioBot TS3',channel:x.channel||'',channelPassword:x.channelPassword||'',serverPassword:x.serverPassword||'',identity:x.identity||prev?.identity||''};});}
  if(Array.isArray(body.spotify)&&user.role!=='viewer') config.instances.spotify=body.spotify;
  config.setupComplete=true;save();rebuildInstancesInBackground();return {ok:true};
});
...
app.post('/api/playlist/:id/item',async(request:any,reply:any)=>{const user=auth(request,reply);if(!user)return;if(user.role==='viewer')return reply.code(403).send({error:'Keine Schreibrechte.'});const p=config.playlists.find((x:any)=>x.id===request.params.id);if(!p)return reply.code(404).send({error:'Playlist nicht gefunden.'});const item=request.body??{};const input=String(item.input??'').trim();if(!input)return reply.code(400).send({error:'Playlist-Eintrag ohne Quelle.'});p.items.push({input,title:String(item.title??input)});save();return{ok:true,playlist:p};});
app.delete('/api/playlist/:id/item/:index',async(request:any,reply:any)=>{const user=auth(request,reply);if(!user)return;if(user.role==='viewer')return reply.code(403).send({error:'Keine Schreibrechte.'});const p=config.playlists.find((x:any)=>x.id===request.params.id);if(!p)return reply.code(404).send({error:'Playlist nicht gefunden.'});const index=Number(request.params.index);if(!Number.isInteger(index)||index<0||index>=p.items.length)return reply.code(404).send({error:'Playlist-Eintrag nicht gefunden.'});p.items.splice(index,1);save();return{ok:true,playlist:p};});
...
app.post('/api/playlist/:id/play',async(request:any,reply:any)=>{const user=auth(request,reply);if(!user)return;if(user.role==='viewer')return reply.code(403).send({error:'Keine Steuerrechte.'});const p=config.playlists.find((x:any)=>x.id===request.params.id);if(!p)return reply.code(404).send({error:'Playlist nicht gefunden.'});const active:any=bot();if(!active)return reply.code(400).send({error:'Keine aktive Instanz.'});for(const item of p.items??[]){try{await active.add(item.input,false);}catch(e){console.error('Playlist-Eintrag',item.input,e);}}if(active.cfg?.id!==config.activeInstance){config.activeInstance=active.cfg.id;save();}return{ok:true,count:p.items?.length??0};});
