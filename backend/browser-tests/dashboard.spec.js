...
test('first-run setup, live dashboard controls, playlists, Discord editor and diagnostics',async({page,context})=>{
  ...
  await page.route('**/api/autoplay/profile/term/validate',async route=>{const term=route.request().postDataJSON().term;await route.fulfill({contentType:'application/json',body:JSON.stringify({valid:true,normalized:term,type:'genre',matched:term,evidence:[`Bekannte Stilrichtung: ${term}`]})})});
  await page.route('**/api/autoplay/profile/styles',async route=>{const body=route.request().postDataJSON();browserAutoplay.profile.preferredStyles=body.preferredStyles;browserAutoplay.profile.blockedStyles=body.blockedStyles;await route.fulfill({contentType:'application/json',body:JSON.stringify({profile:browserAutoplay.profile})})});
  ...
  await expect(page.getByText('Nichts gesperrt – alle Musikrichtungen sind erlaubt.')).toBeVisible();
  await page.getByLabel('Gewünschte Stilrichtungen').fill('Uptempo');
  await page.locator('.profile-style-editor').filter({hasText:'Gewünschte Stilrichtungen'}).getByRole('button',{name:'Prüfen & hinzufügen'}).click();
  await expect(page.locator('.profile-style-chip.preferred')).toContainText('Uptempo');
