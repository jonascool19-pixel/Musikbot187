...
test('first-run setup, live dashboard controls, playlists, Discord editor and diagnostics',async({page,context})=>{
  ...
  const playerCalls=[];let mockPlayer=true;const browserAutoplay={enabled:false,mode:'playlists',playlistIds:[],queueTarget:5,status:'off',detail:'Automatische Wiedergabe ist ausgeschaltet.',profile:{learnedTracks:1,maxTracks:200,maxDurationMinutes:10,totalListens:1,preferredStyles:[],blockedStyles:[],styles:[{name:'Hardstyle',weight:1}],tracks:[{key:'learned-hardstyle',title:'Test Artist – Hardstyle Titel',source:'youtube',listens:1,lastPlayed:1,styles:['Hardstyle']}],top:[]}},mockState={player:{current:{id:'long-track',title:'DIES IST EIN SEHR LANGER TITEL FÜR DAS LAUFENDE RADIO-DISPLAY – SOMMER CEM UND SHIRIN DAVID',source:'youtube',quality:'Beste verfügbare Audioqualität',duration:245},queue:Array.from({length:8},(_,index)=>({id:`queue-${index}`,title:`Wartelistentitel ${index+1}`,source:'youtube'})),volume:40,mode:'queue',paused:false,playing:true,positionSeconds:61,playbackId:7},autoplay:browserAutoplay,settings:{theme:'dark',accent:'#7c3aed',output:'none',outputId:null,marqueeSpeed:45,marqueeTextColor:'#eef2ff',marqueeBackground:'#171e2d'},connections:[],runtimes:[]};
