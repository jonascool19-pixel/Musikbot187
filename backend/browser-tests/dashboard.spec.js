...
test('first-run setup, live dashboard controls, playlists, Discord editor and diagnostics',async({page,context})=>{
  ...
  const playbackCard=page.locator('.playback-card');await playbackCard.locator(':scope > .panel-collapse').click();await expect(playbackCard.getByRole('heading',{name:'Wiedergabe'})).toBeVisible();await expect(playbackCard.locator('#playerCurrent')).toBeVisible();await expect(playbackCard.locator('.now-playing-marquee')).toBeVisible();await expect(playbackCard.locator('.player-controls')).toBeHidden();const centeredPanelArrow=await playbackCard.locator(':scope > .panel-collapse').evaluate(button=>({text:button.textContent,display:getComputedStyle(button).display,placeItems:getComputedStyle(button).placeItems,lineHeight:getComputedStyle(button).lineHeight}));expect(centeredPanelArrow).toEqual({text:'⌄',display:'grid',placeItems:'center',lineHeight:'20px'});await playbackCard.locator(':scope > .panel-collapse').click();await expect(playbackCard.locator('.player-controls')).toBeVisible();
  ...
  await card.getByRole('button',{name:'Instanz einklappen'}).click();await expect(card.getByLabel('Name')).toBeHidden();const centeredInstanceArrow=await card.getByRole('button',{name:'Instanz ausklappen'}).evaluate(button=>({text:button.textContent,display:getComputedStyle(button).display,placeItems:getComputedStyle(button).placeItems,lineHeight:getComputedStyle(button).lineHeight}));expect(centeredInstanceArrow).toEqual({text:'⌄',display:'grid',placeItems:'center',lineHeight:'19px'});await card.getByRole('button',{name:'Instanz ausklappen'}).click();await expect(card.getByLabel('Name')).toBeVisible();
