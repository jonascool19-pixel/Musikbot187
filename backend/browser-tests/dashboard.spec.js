...
test('first-run setup, live dashboard controls, playlists, Discord editor and diagnostics',async({page,context})=>{
  ...
  await page.locator('.profile-style-editor').filter({hasText:'Gewünschte Stilrichtungen'}).getByRole('button',{name:'Hinzufügen'}).click();
  ...
  await page.setViewportSize({width:1360,height:900});await page.getByRole('button',{name:'Abmelden'}).click();await page.getByLabel('Benutzername').fill('browseruser');await page.getByLabel('Passwort',{exact:true}).fill(oneTimePassword);await page.getByRole('button',{name:'Anmelden'}).click();await expect(page.getByRole('heading',{name:'Eigenes Passwort festlegen'})).toBeVisible();await page.getByLabel('Einmal-Passwort').fill(oneTimePassword);await page.getByLabel('Neues Passwort',{exact:true}).fill('browser-user-personal-password');await page.getByLabel('Neues Passwort wiederholen').fill('browser-user-personal-password');await page.getByRole('button',{name:'Passwort speichern & Dashboard öffnen'}).click();await expect(page.locator('#pageTitle')).toHaveText('Dashboard');await expect(page.getByRole('button',{name:'Benutzer & Rechte'})).toHaveCount(0);
