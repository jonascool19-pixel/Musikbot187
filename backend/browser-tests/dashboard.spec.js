...
test('first-run setup, live dashboard controls, playlists, Discord editor and diagnostics',async({page,context})=>{
  ...
  await expect(page.getByText('Summer Cem – Testtitel',{exact:true})).toBeVisible();
  const playlistCard=page.locator('.playlist-card').filter({hasText:'Sommer Hits'});await expect(playlistCard.getByLabel('Playlist Sommer Hits wiederholen')).not.toBeChecked();await expect(playlistCard.getByLabel('Playlist Sommer Hits zufällig abspielen')).not.toBeChecked();await playlistCard.getByLabel('Playlist Sommer Hits wiederholen').check();await playlistCard.getByLabel('Playlist Sommer Hits zufällig abspielen').check();mockPlayer=true;await playlistCard.getByRole('button',{name:'Jetzt abspielen'}).click();await expect.poll(()=>JSON.stringify(playlistPlayBody)).toBe(JSON.stringify({repeat:true,shuffle:true}));await expect(page.locator('#toast')).toContainText('zufällig · in Endlosschleife');
  await page.locator('#nav').getByRole('button',{name:'Dashboard'}).click();await expect(page.getByText('Playlist: Sommer Hits · Zufällig · Endlosschleife',{exact:true})).toBeVisible();await page.locator('#nav').getByRole('button',{name:'Playlists'}).click();await page.getByText('Sommer Hits',{exact:true}).click();mockPlayer=false;
  ...
  await page.getByText('Sommer Hits',{exact:true}).click();
  await expect(page.getByText('Neuer Spotify-Titel',{exact:true})).toBeVisible();
  let syncedPlaylistCard=page.locator('.playlist-card').filter({hasText:'Sommer Hits'});await syncedPlaylistCard.getByRole('button',{name:'Neuer Spotify-Titel aus Sommer Hits entfernen'}).click();await expect(syncedPlaylistCard.getByText('„Neuer Spotify-Titel“ wirklich aus „Sommer Hits“ entfernen? Der Titel wird nur im MusikBot ausgeblendet und nicht bei Spotify gelöscht.',{exact:true})).toBeVisible();await syncedPlaylistCard.getByRole('button',{name:'Jetzt bestätigen'}).click();syncedPlaylistCard=page.locator('.playlist-card').filter({hasText:'Sommer Hits'});await syncedPlaylistCard.getByText('Sommer Hits',{exact:true}).click();await expect(syncedPlaylistCard.getByText('Neuer Spotify-Titel',{exact:true})).toHaveCount(0);await expect(syncedPlaylistCard.getByText('Summer Cem – Testtitel',{exact:true})).toBeVisible();
