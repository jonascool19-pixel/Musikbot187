...
test('first-run setup, live dashboard controls, playlists, Discord editor and diagnostics',async({page,context})=>{
  ...
  let bugReportCalls=0;await page.route(/\/api\/bug-report(?:\/info)?(?:\?.*)?$/,route=>{const url=new URL(route.request().url());if(route.request().method()==='GET'&&url.pathname==='/api/bug-report/info')return route.fulfill({contentType:'application/json',body:JSON.stringify({enabled:true,descriptionMin:20,descriptionMax:2000,maxFiles:3,maxFileBytes:8388608,maxTotalBytes:16777216})});if(route.request().method()==='POST'&&url.pathname==='/api/bug-report'){bugReportCalls++;return route.fulfill({contentType:'application/json',body:JSON.stringify({ok:true,id:'MB187-20260901-BROWSER1',attachments:bugReportCalls===1?1:0})})}return route.fallback()});
  const bellBox=await page.locator('#notificationBell').boundingBox(),bugBox=await page.locator('#bugReportButton').boundingBox();expect(bugBox.x).toBeGreaterThan(bellBox.x);await page.getByRole('button',{name:'🐞 Bug melden'}).click();const bugDialog=page.getByRole('dialog',{name:'Bug melden'});await expect(bugDialog).toBeVisible();await expect(bugDialog.getByText(/ausschließlich an den zentralen privaten/)).toBeVisible();await bugDialog.getByLabel('Fehlerbeschreibung').fill('Beim Start stoppt die Wiedergabe jedes Mal sofort und zeigt keinen Hinweis.');await bugDialog.getByLabel('Bilder oder Videos anhängen').setInputFiles({name:'browser-fehler.png',mimeType:'image/png',buffer:Buffer.from([0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a,0])});await expect(bugDialog.getByText('browser-fehler.png')).toBeVisible();await bugDialog.getByRole('button',{name:'An meinen Support senden'}).click();await expect(bugDialog.getByText(/MB187-20260901-BROWSER1/)).toBeVisible();expect(bugReportCalls).toBe(1);await bugDialog.locator('.dashboard-dialog-close').click();await expect(bugDialog).toBeHidden();await page.getByRole('button',{name:'🐞 Bug melden'}).click();const noMediaDialog=page.getByRole('dialog',{name:'Bug melden'});await noMediaDialog.getByLabel('Fehlerbeschreibung').fill('Dieser Fehlerbericht wird bewusst ohne Bild oder Video versendet.');await noMediaDialog.getByRole('button',{name:'An meinen Support senden'}).click();await expect(noMediaDialog.getByText(/Bilder oder Videos auswählen oder/)).toBeVisible();expect(bugReportCalls).toBe(1);await noMediaDialog.getByLabel('Keine Bilder oder Videos vorhanden').check();await noMediaDialog.getByRole('button',{name:'An meinen Support senden'}).click();await expect(noMediaDialog.getByText(/ohne Bild oder Video/)).toBeVisible();expect(bugReportCalls).toBe(2);await noMediaDialog.locator('.dashboard-dialog-close').click();await expect(noMediaDialog).toBeHidden();
  const powerButton=page.getByRole('button',{name:'Systemsteuerung',exact:true}),powerBox=await powerButton.boundingBox(),settingsBox=await page.getByRole('button',{name:'Systemeinstellungen',exact:true}).boundingBox();expect(await powerButton.textContent()).toBe('⏻');expect(Math.abs(powerBox.width-settingsBox.width)).toBeLessThan(1);expect(Math.abs(powerBox.height-settingsBox.height)).toBeLessThan(1);
  ...
  await page.getByRole('button',{name:'Systemeinstellungen',exact:true}).click();await expect(page.locator('#pageTitle')).toHaveText('System');await expect(page.getByRole('heading',{name:'Update'})).toBeVisible();await expect(page.getByText(/Automatische Prüfung alle 30 Minuten/)).toBeVisible();await page.locator('#nav').getByRole('button',{name:'Dashboard'}).click();
  ...
  await expect(page.getByText('Automatik-Modus · Persönlicher Musik-Mix',{exact:true})).toBeVisible();
  ...
  await page.getByRole('button',{name:'Einschalten',exact:true}).click();await expect(page.getByText('Automatische Wiedergabe · EIN',{exact:true})).toBeVisible();await expect(page.getByText('Automatik-Modus · Playlist: Browser-Mix',{exact:true})).toBeVisible();
  ...
  const playbackCard=page.locator('.playback-card');await playbackCard.locator(':scope > .panel-collapse').click();await expect(playbackCard.getByRole('heading',{name:'Wiedergabe'})).toBeVisible();await expect(playbackCard.locator('#playerCurrent')).toBeVisible();await expect(playbackCard.locator('.now-playing-marquee')).toBeVisible();await expect(playbackCard.locator('.player-controls')).toBeHidden();const centeredPanelArrow=await playbackCard.locator(':scope > .panel-collapse').evaluate(button=>{const buttonStyle=getComputedStyle(button),arrowStyle=getComputedStyle(button,'::before');return {text:button.textContent,display:buttonStyle.display,placeItems:buttonStyle.placeItems,fontSize:buttonStyle.fontSize,arrowWidth:arrowStyle.width,arrowHeight:arrowStyle.height}});expect(centeredPanelArrow).toEqual({text:'',display:'grid',placeItems:'center',fontSize:'0px',arrowWidth:'9px',arrowHeight:'9px'});await playbackCard.locator(':scope > .panel-collapse').click();await expect(playbackCard.locator('.player-controls')).toBeVisible();
  mockPlayer=false;await page.waitForTimeout(3200);

  let importedPlaylist=null,playlistPlayBody=null;
  await page.route('**/api/playlists',route=>route.request().method()==='GET'&&importedPlaylist?route.fulfill({contentType:'application/json',body:JSON.stringify([importedPlaylist,...Array.from({length:12},(_,index)=>({id:`browser-list-${index}`,name:`Browser Liste ${index+1}`,items:[],playbackRepeat:false,playbackShuffle:false}))])}):route.fallback());
  ...
  await page.route('**/api/playlists/spotify-browser/items/*',async route=>{const itemIndex=Number(new URL(route.request().url()).pathname.split('/').at(-1)),[removed]=importedPlaylist.items.splice(itemIndex,1);return route.fulfill({contentType:'application/json',body:JSON.stringify({playlist:importedPlaylist,removed})})});
  await page.route('**/api/playlists/spotify-browser/play',async route=>{playlistPlayBody=route.request().postDataJSON();mockState.player={...mockState.player,current:importedPlaylist.items[0],queue:importedPlaylist.items.slice(1),playlistPlayback:{playlistId:importedPlaylist.id,playlistName:importedPlaylist.name,repeat:playlistPlayBody.repeat,shuffle:playlistPlayBody.shuffle}};return route.fulfill({contentType:'application/json',body:JSON.stringify(mockState.player)})});
  ...
  await expect(page.getByText('Sommer Hits',{exact:true})).toBeVisible();const playlistLibraryScroll=await page.getByRole('region',{name:'Playlist-Liste'}).evaluate(element=>({overflowY:getComputedStyle(element).overflowY,overflowX:getComputedStyle(element).overflowX,scrollHeight:element.scrollHeight,clientHeight:element.clientHeight}));expect(playlistLibraryScroll.overflowY).toBe('auto');expect(playlistLibraryScroll.overflowX).toBe('hidden');expect(playlistLibraryScroll.scrollHeight).toBeGreaterThan(playlistLibraryScroll.clientHeight);
  let playlistCard=page.locator('.playlist-card').filter({hasText:'Sommer Hits'});await playlistCard.getByRole('button',{name:'Playlist öffnen'}).click();let playlistDetail=page.getByRole('dialog',{name:'Sommer Hits'});await expect(playlistDetail).toBeVisible();await expect(playlistDetail.getByText('Summer Cem – Testtitel',{exact:true})).toBeVisible();const detailScroll=await playlistDetail.getByRole('region',{name:'Titel von Sommer Hits'}).evaluate(element=>getComputedStyle(element).overflowY);expect(detailScroll).toBe('auto');
  await expect(playlistDetail.getByLabel('Playlist Sommer Hits wiederholen')).not.toBeChecked();await expect(playlistDetail.getByLabel('Playlist Sommer Hits zufällig abspielen')).not.toBeChecked();await playlistDetail.getByLabel('Playlist Sommer Hits wiederholen').check();await playlistDetail.getByLabel('Playlist Sommer Hits zufällig abspielen').check();mockPlayer=true;await playlistDetail.getByRole('button',{name:'Jetzt abspielen'}).click();await expect.poll(()=>JSON.stringify(playlistPlayBody)).toBe(JSON.stringify({repeat:true,shuffle:true}));await expect(page.locator('#toast')).toContainText('zufällig · in Endlosschleife');await playlistDetail.getByRole('button',{name:'Fenster schließen'}).click();
  await page.locator('#nav').getByRole('button',{name:'Dashboard'}).click();await expect(page.getByText('Playlist: Sommer Hits · Zufällig · Endlosschleife',{exact:true})).toBeVisible();await page.locator('#nav').getByRole('button',{name:'Playlists'}).click();playlistCard=page.locator('.playlist-card').filter({hasText:'Sommer Hits'});await playlistCard.getByRole('button',{name:'Playlist öffnen'}).click();playlistDetail=page.getByRole('dialog',{name:'Sommer Hits'});mockPlayer=false;
  await playlistDetail.getByRole('button',{name:'Play',exact:true}).click();await expect.poll(()=>playerCalls.filter(call=>call.url.includes('/api/player/queue')).map(call=>call.body).join('\n')).toContain('"playlistOriginName":"Sommer Hits"');
  await playlistDetail.getByLabel('Spotify-Abgleich für Sommer Hits').selectOption('5');await expect(playlistDetail.getByLabel('Spotify-Abgleich für Sommer Hits')).toHaveValue('5');await playlistDetail.getByRole('button',{name:'Jetzt mit Spotify abgleichen'}).click();await expect(playlistDetail.getByText('Neuer Spotify-Titel',{exact:true})).toBeVisible();
  await playlistDetail.getByRole('button',{name:'Neuer Spotify-Titel aus Sommer Hits entfernen'}).click();await expect(playlistDetail.getByText('„Neuer Spotify-Titel“ wirklich aus „Sommer Hits“ entfernen? Der Titel wird nur im MusikBot ausgeblendet und nicht bei Spotify gelöscht.',{exact:true})).toBeVisible();await playlistDetail.getByRole('button',{name:'Jetzt bestätigen'}).click();await expect(playlistDetail.getByText('Neuer Spotify-Titel',{exact:true})).toHaveCount(0);await expect(playlistDetail.getByText('Summer Cem – Testtitel',{exact:true})).toBeVisible();await playlistDetail.getByRole('button',{name:'Fenster schließen'}).click();
  ...
  await page.route('**/api/autoplay/profile/term/validate',async route=>{const {term,kind}=route.request().postDataJSON(),artist=kind==='artist',candidate=artist?'Rammstein':term==='Hard'?'Hardstyle':term,valid=artist?term==='Rammstein':term!=='Hard',suggestions=valid?[{value:candidate,type:artist?'artist':'genre',verified:true,evidence:`Geprüft: ${candidate}`}]:[{value:candidate,type:artist?'artist':'genre',verified:true,evidence:`Vorschlag: ${candidate}`}];await route.fulfill({contentType:'application/json',body:JSON.stringify({valid,normalized:term,type:artist?'artist':'genre',matched:valid?candidate:term,evidence:valid?[`Geprüft: ${candidate}`]:[],suggestions})})});
  await page.route('**/api/autoplay/profile/styles',async route=>{const body=route.request().postDataJSON();browserAutoplay.profile.preferredStyles=body.preferredStyles;browserAutoplay.profile.preferredArtists=body.preferredArtists;browserAutoplay.profile.blockedStyles=body.blockedStyles;await route.fulfill({contentType:'application/json',body:JSON.stringify({profile:browserAutoplay.profile})})});
  let profileLearnBody=null;await page.route('**/api/autoplay/profile/playlists',async route=>{profileLearnBody=route.request().postDataJSON();const learned={key:'spotify-track',title:'Summer Cem – Testtitel',source:'spotify',listens:1,lastPlayed:3,styles:[]};browserAutoplay.profile.tracks=[learned];browserAutoplay.profile.learnedTracks=1;browserAutoplay.profile.totalListens=1;await route.fulfill({contentType:'application/json',body:JSON.stringify({profile:browserAutoplay.profile,playlistCount:1,scanned:1,accepted:1,added:1,existing:0,ignored:0})})});
  await page.route('**/api/autoplay/profile/track/block',async route=>{browserAutoplay.profile.blockedStyles=[...new Set([...browserAutoplay.profile.blockedStyles,'Hardstyle'])];browserAutoplay.profile.tracks=browserAutoplay.profile.tracks.filter(track=>!track.styles.includes('Hardstyle'));browserAutoplay.profile.learnedTracks=browserAutoplay.profile.tracks.length;await route.fulfill({contentType:'application/json',body:JSON.stringify({profile:browserAutoplay.profile,added:['Hardstyle']})})});
  await page.route('**/api/autoplay/profile',async route=>{if(route.request().method()!=='DELETE')return route.fallback();browserAutoplay.profile.tracks=[];browserAutoplay.profile.learnedTracks=0;browserAutoplay.profile.totalListens=0;await route.fulfill({contentType:'application/json',body:JSON.stringify({profile:browserAutoplay.profile})})});
  ...
  await expect(page.getByText('Noch kein Musikprofil vorhanden.')).toBeVisible();
  await expect(page.locator('#toast')).toContainText('Gelernte Titel wurden gelöscht.');
  await page.getByRole('tab',{name:'Aus Playlists lernen'}).click();await expect(page.getByLabel('Sommer Hits für das Musikprofil auswählen')).toBeVisible();await page.getByLabel('Sommer Hits für das Musikprofil auswählen').check();await page.getByRole('button',{name:'Auswahl analysieren'}).click();await expect.poll(()=>JSON.stringify(profileLearnBody)).toBe(JSON.stringify({playlistIds:['spotify-browser']}));await expect(page.getByText('1 neue Titel gelernt · 0 bereits bekannt · 1 Titel geprüft.')).toBeVisible();await page.getByRole('tab',{name:'Profil & Vorlieben'}).click();await expect(page.getByText('Summer Cem – Testtitel',{exact:true})).toBeVisible();
  ...
  await card.getByRole('button',{name:'Instanz einklappen'}).click();await expect(card.getByLabel('Name')).toBeHidden();const centeredInstanceArrow=await card.getByRole('button',{name:'Instanz ausklappen'}).evaluate(button=>{const buttonStyle=getComputedStyle(button),arrowStyle=getComputedStyle(button,'::before');return {text:button.textContent,display:buttonStyle.display,placeItems:buttonStyle.placeItems,fontSize:buttonStyle.fontSize,arrowWidth:arrowStyle.width,arrowHeight:arrowStyle.height}});expect(centeredInstanceArrow).toEqual({text:'',display:'grid',placeItems:'center',fontSize:'0px',arrowWidth:'9px',arrowHeight:'9px'});await card.getByRole('button',{name:'Instanz ausklappen'}).click();await expect(card.getByLabel('Name')).toBeVisible();
  ...
  await page.locator('#notificationBell').click();await expect(page.locator('#notificationPanel')).toBeVisible();await expect(page.locator('#notificationPanel .notification-item').first()).toBeVisible();await expect(page.locator('#notificationPanel').getByRole('button',{name:'Alle gelesen'})).toBeVisible();await page.locator('#notificationPanel').getByRole('button',{name:'Alle löschen'}).click();await expect(page.locator('#notificationPanel').getByText('Keine Benachrichtigungen vorhanden.')).toBeVisible();await expect(page.locator('#notificationBadge')).toBeHidden();await page.locator('#notificationBell').click();
  await page.locator('#nav').getByRole('button',{name:'Fehlermeldungen'}).click();
  ...
  const ownerCard=page.locator('.collapsible-user-card').filter({hasText:'browseradmin'});await expect(ownerCard).not.toHaveAttribute('open','');const centeredUserArrow=await ownerCard.locator(':scope > summary').evaluate(summary=>{const arrow=getComputedStyle(summary,'::after');return {width:arrow.width,height:arrow.height,content:arrow.content}});expect(centeredUserArrow).toEqual({width:'9px',height:'9px',content:'""'});await ownerCard.getByText('browseradmin',{exact:true}).click();await expect(ownerCard).toHaveAttribute('open','');await ownerCard.getByText('browseradmin',{exact:true}).click();await expect(ownerCard).not.toHaveAttribute('open','');
  const createUser=page.locator('.user-create');await createUser.getByLabel('Benutzername').fill('browseruser');await createUser.getByRole('button',{name:'Benutzer anlegen',exact:true}).click();await expect(page.getByRole('heading',{name:'browseruser wurde angelegt'})).toBeVisible();const oneTimePassword=await page.getByLabel('Temporäres Einmal-Passwort').inputValue();expect(oneTimePassword).toMatch(/^Mb187-[A-Za-z0-9_-]{20}$/);await page.getByRole('button',{name:'Ich habe es gespeichert'}).click();
  let userCard=page.locator('.collapsible-user-card').filter({hasText:'browseruser'});await expect(userCard).toBeVisible();await expect(userCard).not.toHaveAttribute('open','');await expect(userCard.getByText('Passwortwechsel offen',{exact:true})).toBeVisible();await userCard.getByText('browseruser',{exact:true}).click();await expect(userCard).toHaveAttribute('open','');await userCard.getByLabel('Playlists verwalten').check();await userCard.getByRole('button',{name:'Änderungen speichern'}).click();
  userCard=page.locator('.collapsible-user-card').filter({hasText:'browseruser'});await expect(userCard).not.toHaveAttribute('open','');await userCard.getByText('browseruser',{exact:true}).click();await expect(userCard.getByLabel('Playlists verwalten')).toBeChecked();
  ...
  await page.setViewportSize({width:1360,height:900});await page.getByRole('button',{name:'Abmelden'}).click();await page.getByLabel('Benutzername').fill('browseruser');await page.getByLabel('Passwort',{exact:true}).fill(oneTimePassword);await page.getByRole('button',{name:'Anmelden'}).click();await expect(page.getByRole('heading',{name:'Eigenes Passwort festlegen'})).toBeVisible();const currentPasswordField=page.getByLabel('Einmal-Passwort'),newPasswordField=page.getByLabel('Neues Passwort',{exact:true}),repeatPasswordField=page.getByLabel('Neues Passwort wiederholen');await currentPasswordField.fill(oneTimePassword);await newPasswordField.fill('browser-user-personal-password');await repeatPasswordField.fill('browser-user-personal-password');await newPasswordField.evaluate(element=>element.dataset.browserIdentity='preserved');await page.waitForTimeout(4200);await expect(newPasswordField).toHaveValue('browser-user-personal-password');await expect(newPasswordField).toHaveAttribute('data-browser-identity','preserved');await expect(currentPasswordField).toHaveValue(oneTimePassword);await expect(repeatPasswordField).toHaveValue('browser-user-personal-password');await page.getByRole('button',{name:'Passwort speichern & Dashboard öffnen'}).click();await expect(page.locator('#pageTitle')).toHaveText('Dashboard');await expect(page.getByRole('button',{name:'Benutzer & Rechte'})).toHaveCount(0);
