(() => {
  ...
  let unregisterMusicTab = null;
  function syncMusicTab() {
    if (can('music.manage')) {
      if (!unregisterMusicTab && window.MusikBotNavigation?.registerExtraTab) {
        unregisterMusicTab = window.MusikBotNavigation.registerExtraTab({ id:'music', label:'🎼 Musik', title:'Eigene Musikdateien verwalten', render:renderMusic });
      }
      window.MusikBotNavigation?.refresh?.();
    } else if (unregisterMusicTab) {
      unregisterMusicTab();
      unregisterMusicTab = null;
      if (document.body.dataset.currentTab === 'music') document.body.dataset.currentTab = 'player';
    }
  }
  window.__musikbotSyncMusicTab = syncMusicTab;
  window.__musikbotEnhanceMusicResults = enhanceSearchResults;
  window.addEventListener('musikbot:auth-changed', syncMusicTab);
  syncMusicTab();
