(() => {
  ...
  const activeInstance = state => (state?.instances || []).find(x => x.id === state.activeInstance) || (state?.instances || []).find(x => x.connected) || null;
  ...
  function renderNowPlaying(instance) {
    ...
    const elapsed = instance.elapsedSeconds || 0;
    const current = tile.querySelector('.now-playing');
    if (current) current.innerHTML = `<div class="cover">${cover}</div><div><h2>${esc(title)}</h2><p>${esc(instance.name || 'Keine aktive Instanz')}</p><div class="meta"><span>Quelle</span><b>${esc(source)}</b><span>Zeit</span><b>${formatTime(elapsed)}</b><span>Status</span><b class="${instance.playing ? 'ok' : 'bad'}">${status}</b></div></div>`;
    if (!tile.querySelector('.stable-volume')) {
      ...
      const volume = document.querySelector('#stableVolume'); if (volume && document.activeElement !== volume) volume.value = Number(instance.volume ?? 80);
    }
  }

  function updateNetwork(state) {
    const net = state?.system || {};
    ...
    const systemTile = document.querySelector('.system-tile .system-mini');
    if (systemTile) systemTile.innerHTML = `<span>NET ↓ ${formatBytes(net.networkRx || 0)}/s</span><span>↑ ${formatBytes(net.networkTx || 0)}/s</span>`;
  }
  ...
  function injectPlaylistOpeners(playlists) {
    document.querySelectorAll('.playlist-card').forEach(card => {
      ...
      const playlist = playlists.find(p => p.name === title);
      if (!playlist) return;
      const actions = card.querySelector('div:last-child');
      if (!actions) return;
      const button = document.createElement('button');
      button.className = 'stable-open-playlist';
      button.textContent = 'Öffnen';
      button.addEventListener('click', () => openPlaylist(playlist));
      actions.insertBefore(button, actions.firstChild);
    });
  }

  function openPlaylist(playlist) {
    const existing = document.querySelector('#stablePlaylistModal'); existing?.remove();
    ...
    modal.querySelectorAll('[data-del]').forEach(button => button.addEventListener('click', async () => { await api(`/api/playlist/${playlist.id}/item/${button.dataset.del}`, {method:'DELETE'}); modal.remove(); await refreshStable(); document.querySelector('[data-nav="playlists"]')?.click(); }));
  }

  const observer = new MutationObserver(() => { ensureEditorButton(); lockDashboard(); injectNetworkTab(); });
  observer.observe(document.body, { childList: true, subtree: true });
  setInterval(refreshStable, 3000);
  refreshStable();
})();
