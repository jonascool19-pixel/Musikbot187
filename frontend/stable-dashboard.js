(() => {
  ...
  function ensureEditorButton() {
    ...
    if (!button) {
      ...
      button.id = 'stableDashboardEdit'; button.className = 'icon-btn'; button.type = 'button';
      button.addEventListener('click', () => { editing = !editing; button.textContent = editing ? '✓' : '✦'; button.title = editing ? 'Bearbeiten beenden' : 'Dashboard bearbeiten'; document.body.classList.toggle('dashboard-editing', editing); lockDashboard(); });
      top.insertBefore(button, document.querySelector('#settingsQuick') || null);
    }
    button.textContent = editing ? '✓' : '✦'; button.title = editing ? 'Bearbeiten beenden' : 'Dashboard bearbeiten'; lockDashboard();
  }

  document.addEventListener('dragstart', e => { const tile = e.target.closest?.('[data-tile]'); if (tile && !editing) { e.preventDefault(); e.stopImmediatePropagation(); } }, true);
  document.addEventListener('dragover', e => { const tile = e.target.closest?.('[data-tile]'); if (tile && !editing) { e.preventDefault(); e.stopImmediatePropagation(); } }, true);
  document.addEventListener('drop', e => { const tile = e.target.closest?.('[data-tile]'); if (tile && !editing) { e.preventDefault(); e.stopImmediatePropagation(); } }, true);
  ...
  function renderNowPlaying(instance) {
    ...
    const cover = instance.coverUrl ? `<img src="${esc(instance.coverUrl)}" alt="" loading="lazy" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'cover-fallback',textContent:'♫'}))">` : '<div class="cover-fallback">♫</div>';
    ...
    if (!tile.querySelector('.stable-volume')) {
      ...
      document.querySelector('#stableVolume')?.addEventListener('input', async e => { try { await api('/api/control',{method:'POST',body:JSON.stringify({action:'volume',value:Number(e.target.value)})}); } catch {} });
    } else { const volume=document.querySelector('#stableVolume'); if(volume&&document.activeElement!==volume) volume.value=Number(instance.volume??80); }
  }

  function updateNetwork(s) {
    const net=s?.system||{}; const top=document.querySelector('#topNet'); if(top) top.textContent=`NET ↓ ${formatBytes(net.networkRx||0)}/s ↑ ${formatBytes(net.networkTx||0)}/s`;
    const tile=document.querySelector('.system-tile .system-mini'); if(tile) tile.innerHTML=`<span>NET ↓ ${formatBytes(net.networkRx||0)}/s</span><span>↑ ${formatBytes(net.networkTx||0)}/s</span>`;
  }

  async function refreshStable() {
    try { const fresh=await api('/api/state'); lastState=fresh; if(location.pathname==='/'){ renderNowPlaying(activeInstance(fresh)); updateNetwork(fresh); ensureEditorButton(); } injectNetworkTab(); injectPlaylistOpeners(fresh.playlists||[]); } catch {}
  }
  ...
  function injectNetworkTab() {
    ...
    tabs.querySelector('[data-settab="errors"]')?.after(tab) || tabs.appendChild(tab);
  ...
  async function renderNetworkSettings(target) {
    ...
    try {
      ...
      document.querySelector('#stableNetworkInterface').addEventListener('change',async e=>{await api('/api/settings',{method:'PUT',body:JSON.stringify({settings:{networkInterface:e.target.value}})});renderNetworkSettings(target);});
    }catch(error){target.innerHTML=`<div class="page-panel"><div class="empty">${esc(error.message)}</div></div>`;}
  }

  function injectPlaylistOpeners(playlists) {
    document.querySelectorAll('.playlist-card').forEach(card=>{ if(card.querySelector('.stable-open-playlist'))return; const title=card.querySelector('h3')?.textContent?.trim(); const p=playlists.find(x=>x.name===title); if(!p)return; const actions=card.querySelector('div:last-child'); if(!actions)return; const b=document.createElement('button'); b.className='stable-open-playlist'; b.textContent='Öffnen'; b.addEventListener('click',()=>openPlaylist(p)); actions.insertBefore(b,actions.firstChild); });
  }
  function openPlaylist(playlist) {
    document.querySelector('#stablePlaylistModal')?.remove(); const modal=document.createElement('div'); modal.id='stablePlaylistModal'; modal.className='stable-modal-backdrop';
    ...
    document.body.appendChild(modal); modal.querySelector('#stableClose').onclick=()=>modal.remove(); modal.addEventListener('click',e=>{if(e.target===modal)modal.remove();}); modal.querySelectorAll('[data-del]').forEach(b=>b.addEventListener('click',async()=>{await api(`/api/playlist/${playlist.id}/item/${b.dataset.del}`,{method:'DELETE'});modal.remove();await refreshStable();document.querySelector('[data-nav="playlists"]')?.click();}));
  }

  const observer=new MutationObserver(()=>{ensureEditorButton();lockDashboard();injectNetworkTab();}); observer.observe(document.body,{childList:true,subtree:true}); setInterval(refreshStable,3000); refreshStable();
})();
