(() => {
  ...
  function navButton() {
    ...
    if (!nav || nav.querySelector('[data-tab="music"]')) return;
    const button = document.createElement('button');
    button.className = 'navbtn';
    button.dataset.tab = 'music';
    button.textContent = '🎼 Musik';
    nav.insertBefore(button, nav.querySelector('.danger') || null);
    button.onclick = () => renderMusic();
  }
  function playlistDialog() {
    return api('/api/playlists').then(playlists => {
      if (!playlists.length) { note('Keine Playlist vorhanden.'); return null; }
      const text = playlists.map((p, i) => `${i + 1}: ${p.name}`).join('\n');
  ...
  async function addToPlaylist(item) {
    try {
      const playlist = await playlistDialog();
      if (!playlist) return;
      await post(`/api/playlists/${encodeURIComponent(playlist.id)}/items`, { items: [{ id: crypto.randomUUID(), title: item.title, url: item.url, source: item.source || 'file', artist: item.artist || '' }] });
      note(`„${playlist.name}“ wurde hinzugefügt.`);
